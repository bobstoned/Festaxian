...
import fester.Festaxian.BossBug.BossBugFactory;
import java.util.Random;
import fester.Festaxian.BossBug.BossBugSprite;
...
public class GalaxianGameEngine {
  ...
  OnGameOverListener onGameOverListener = null;

  // This function is called to indicate game over to the parent
  private void OnGameOver(){
    // Check if the Listener was set, otherwise we'll get an Exception when we try to call it
    if(onGameOverListener!=null) {
      // raise the game over event handlers
  ...
  private BossBugFactory bossBugFactory;
  private BossBugSprite bossBugShip;
  ...
  private SpriteTimer gameOverTimer = new SpriteTimer(); // Wait to display title screen
  ...
  public void init(Context context)
  {
    ...
    bossBugFactory = new BossBugFactory(this.context);
    bossBugFactory.GameArea = gameArea;
    bossBugFactory.PixelSize = pixelSize;
    //bossBugShip = bossBugFactory.CreateBossBug(50, 10, 200);
    bossBugFactory.AddShip(50, 10, 50);
    bossBugFactory.AddShip(60, 20, 100);
    //bossBugFactory.AddShip(70, 20, 150);
    //bossBugFactory.AddShip(80, 20, 200);
    InitSounds();
    curFleetNo= 1;
  }
  ...
  public void SetScreenDimensions(int width, int height) {
    ...
    if (height >= 1700 && (width >= 1000 && width<= 1080)) {
      specificSize = true;
      formation.gap = 3;
      //pixelSize = 2.3;
      pixelSize = 5.5;
      ...
      gameArea.left = 20;
      gameArea.top = 20;
      gameArea.right = width-20;
      gameArea.bottom = 1100;
      galArea.left = gameArea.left;
      galArea.top = 60;
      galArea.right = gameArea.right;
      galArea.bottom = gameArea.bottom;
  ...
  private void CreatePlayerShip() {
    ...
    playerShip.Movement.delay = 10;
    playerShip.Movement.setMovementStep(0.3 * pixelSize);
    playerShip.Animation.Frames = new Point[1] ;
    playerShip.Animation.Frames[0] = new Point(0,0);
    playerShip.Position = new Point(gameArea.left + playerShip.Extent.width()/2,gameArea.bottom-( (int)(playerShip.logicalHeight * pixelSize) ) );
  ...
  public Galaxian CreateGalaxian(int shipType, long attackDelay, double attackStep) {
    ...
    galaxian.logicalHeight = 12;
    galaxian.logicalWidth = 12;
  ...
  public void DrawGameFrame(Canvas canvas) {
    ...
    if (newPlayerReadyTimer.Enabled && curPlayerShip <= maxPlayerShips) {
    ...
    // Start the game over timer on game over
    // waits 10 seconds to show the title screen
    if (!gameOverTimer.Enabled && curPlayerShip > maxPlayerShips && !GameOver) {
      GameOver = true;
      gameOverTimer.delay =  10000;
      gameOverTimer.Reset();
  ...
  private void MoveBossBug(Canvas canvas) {
    bossBugFactory.MoveSprites(canvas);
    // Check for collisions with vulnerable areas of boss ships
    for(int i=0;i<bossBugFactory.Ships.size();i++) {
      // Is the player missile colliding with the galaxian sprite
      if (!playerMissile.IsDyingOrDead() && !bossBugFactory.Ships.get(i).IsDyingOrDead())
        if (bossBugFactory.Ships.get(i).IsCollision(playerMissile.GetBoundingBox())) {
          playerMissile.Dying = true;
        }
    }
    /*
    if (!bossBugShip.Drawing && !bossBugShip.Dead) {
      bossBugShip.AdvanceFrame() ;
      bossBugShip.Move(false);
    }
    if (bossBugShip.Dead) {
      // reset so the next wave can start
      endOfLevel = false;
    ...
    Point frame;
    frame = bossBugShip.GetFrame();
    bossBugShip.SpriteRect = bossBugFactory.Sheet.GetSpriteRectAt(frame.x, frame.y);
    bossBugShip.DrawSprite(canvas);
    */
  }
  ...
  //When the base ship is hit, the score is higher
  //if the wingmen are hit first
  private void DrawAttackGroupBaseShipScore(GalaxianAttackGroup group,  Canvas canvas) {
    if (group.BaseShip.Dead && group.BaseShipDeathScoreSprite.Dying) {
      ...
      scoreSheet.DrawSprite(scoreSheetBitmap, canvas, group.BaseShipDeathScoreSprite.SpriteRect, group.BaseShipDeathScoreSprite.Position, pixelSize, 16, 16);
    }
  }
