...
public class SpriteFormation {
  ...
  public void Move() {
    ...
    if (AllDocked() || GetRemainingShipCount() <= 5) {
        DisableAttack();
        EnableAttack();
    }
  }
    ...
    for(int c=0; c<cols; c++)
      for(int r=0; r<rows; r++) {
        ...
        if (sprite != null && !sprite.Dead && !sprite.Dying) {
          ...
          // Enable all ships to attack together
          if (GetRemainingShipCount() <= 5) {
            // override the docking status and attack continuously
            if (sprite.getStatus() == Galaxian.AttackStatus.Docking) {
              // terminate docking sequence early
              sprite.dockingMovement.Complete = true;
            }
            else {
              //Attack now.
              sprite.AttackTimer.Enabled = true;
              sprite.AttackTimer.delay =0;
              sprite.AttackTimer.Expired();
            }
          }
        }
  ...
  private int GetRemainingShipCount() {
    int count = 0;
    Galaxian sprite;
    for (int c = 0; c < cols; c++)
      for (int r = 0; r < rows; r++) {
        sprite = matrix[c][r];
        if (sprite != null && !sprite.IsDyingOrDead()) {
          count++;
        }
      }
    return count;
  ...
  public void SetFormationDirection(Rect gameArea) {
    ...
    int rightEdge = gameArea.right -1;//- ((int)(leftAttackShip.getLogicalRect().width()*pixelSize));
    if (fRect.right >= rightEdge && leftAttackShip.Movement.Direction == Movement.MovementDirection.Right) {
    ...
    for(int w=0; w<cols; w++)
      for(int h=0; h<rows; h++) {
        ...
        if (galaxian != null && !galaxian.Dead) {
          boundRect = matrix[w][h].GetFormationBoundingBox(12, 12);
